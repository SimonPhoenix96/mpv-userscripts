...
-- default location is `%HOMEDRIVE%\\%HOMEPATH%\\Videos\\bumps` 
...
if(bumpWorthy ) then
   if not usingLinux then
      ...
      -- linux compatibilty
      webmDir = webmDir .. "/bumpworthy"
      webUrl = "https://raw.githubusercontent.com/SimonPhoenix96/random/main/bump-links/bumpworthy/bumpworthy-links.txt"
   end
else
   if not usingLinux then
      ...
      -- linux compatibilty
      webmDir = webmDir .. "/4chan"
      webUrl = "https://boards.4channel.org/wsg/thread/4069030"
   end
...
function downloadWebms()
   if(ranDownloadedWebms == false) then
      ...
      then
         -- Windows Version
         if(bumpWorthy) then
           print('powershell.exe  -file "' .. script_path() .. 'bump-scraper.ps1" "' .. "bumpworthy" .. '" "' .. webmDir .. '" ' .. tostring(streamMode))
            os.execute('powershell.exe  -ExecutionPolicy Bypass -file  "' .. script_path() .. 'bump-scraper.ps1" "bumpworthy" "' .. webmDir .. '" ' .. tostring(streamMode))
         else
            -- print('powershell.exe -file "' .. script_path() .. 'bump-scraper.ps1" "' .. "4chan" .. '" "' .. webmDir .. '" ' .. tostring(streamMode))
            -- -- print('if(((Get-Process mpv -ErrorAction SilentlyContinue) -eq $null)){powershell.exe -windowstyle hidden -file "' .. script_path() .. 'bump-scraper.ps1" "' .. "4chan" .. '" "' .. webmDir .. '" ' .. tostring(streamMode) .. "}" )
            -- os.execute('if(-not((Get-Process mpv -ErrorAction SilentlyContinue) -eq $null)){powershell.exe -windowstyle hidden -file "' .. script_path() .. 'bump-scraper.ps1" "' .. "4chan" .. '" "' .. webmDir .. '" ' .. tostring(streamMode) .. "}" ) -- change regex pattern in bump-scraper.ps1 to website other than the chan
            os.execute('powershell.exe  -ExecutionPolicy Bypass -file "' .. script_path() .. 'bump-scraper.ps1" "' .. "4chan" .. '" "' .. webmDir .. '" ' .. tostring(streamMode))
         end
      else
         --Linux Version
